...
| 15 | 执行用时口径 | 提交→云端生成结束（终态检测时先刻录 finished_at） | 下载成品/补录产物排在打点之后，不污染用时；任务表列优先最近一次 completed，无则取最近终态 | `workers/poll.py: _finalize_completed` + `store/task_store.py: attach_run_result` |
| 16 | 素材提取直链安全与配置 | 接口返回的直链只认 `api_text/{video,image}.txt` 域名白名单；白名单**不随代码分发**，使用者首次用工具时导入（缺失弹引导）；接口 **UID/Key 界面可改**（存 `api_text/api_config.json` 覆盖程序默认），**接口地址不在界面暴露**——只存维护人的 `core/config_local.py`，保存时 `base=None` 保留原值 | 仍未导入齐则不启动执行；json 损坏默默回退默认；每次执行重读名单 | `video_text_tools/material_extract.py` + `gui/tool_panels.py: MaterialPanel` |
