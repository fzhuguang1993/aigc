...
双击运行，第一次会进入配置向导（GUI 是同内容的弹窗）。**打包机已经把接口
地址编进了 exe**，所以普通同事只需要填一个姓名：

```
1) 请输入你的姓名（用于生成视频文件命名）: 张三
2) 接口地址已由维护人内置（共 7 条线路），无需填写。
✓ 配置完成！
```

- 只有内置地址为空时（例如公司换了服务），才会让你粘接口地址，**至少 1 个**
- 配置存在隐藏配置家 `CONFIG_DIR/config.json`（Windows `%APPDATA%\AIGC视频助手`），
  **以后启动不再询问**；想重新配置：删掉那个 `config.json` 再启动即可
- 开发机（跑源码）只往 `config.json` 里记姓名，线路每次启动现读 `core/config_local.py`，
  改内置地址立刻生效（不会被旧副本盖住）
...
| 日志报 `port=80` 连接超时 / `Max retries exceeded` | 接口地址漏了端口，改为 `http://<IP>:7860` 后重启（或删除 `config.json` 重走向导） |
...
- `config.json` 加载优先级：配置家 `config.json` 里的 `accounts` → **exe 内置线路**
  （`core/config_local.py` 的 `ACCOUNTS`，没 accounts 键时每次现读）
